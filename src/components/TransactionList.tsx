...
export function TransactionList({
  ...
  const bulkRecommendation = useMemo(() => {
    ...
    const recs = selectedTransactions.map((t) =>
      getRecommendedCategoryFromTransaction(t)
    );
    const first = recs[0];
    if (!first) return null;
    if (recs.every((r) => r === first)) return first;
    return null;
  ...
  return (
    <div className="rounded-xl border border-[var(--border)] bg-[var(--card)] overflow-hidden transition-shadow duration-200 hover:shadow-lg">
      <div className="p-3 sm:p-4 border-b border-[var(--border)] space-y-3">
        <div className="flex flex-col sm:flex-row flex-wrap items-stretch sm:items-center gap-2">
          <div className="relative flex-1 min-w-0 w-full sm:min-w-[180px]">
            ...
            <Input
              ...
              className="pl-9 h-10 sm:h-9 bg-[var(--secondary)] w-full"
            />
          </div>
            ...
            {(["all", "uncategorized", "income", "expense"] as const).map(
              (f) => (
                <Button
                  ...
                  size="sm"
                  className="min-h-10 transition-all duration-200"
                  onClick={() => setFilter(f)}
                >
                  {f === "all" ? "All" : f === "uncategorized" ? "Uncat." : f === "income" ? "Income" : "Expenses"}
                </Button>
              )
            )}
          </div>
        ...
        {selectedCount > 0 && (
          <div className="flex flex-col sm:flex-row sm:flex-wrap items-stretch sm:items-center gap-2 py-2 px-3 rounded-lg bg-[var(--primary)]/10 border border-[var(--primary)]/30">
            ...
            <div className="flex flex-wrap gap-2">
                    ...
                    <div className="absolute left-0 top-full mt-1 z-20 py-1 rounded-lg border border-[var(--border)] bg-[var(--card)] shadow-xl min-w-[180px] max-h-64 overflow-y-auto">
                      {bulkRecommendation && (
                        <>
                          ...
                          <button
                            type="button"
                            className="flex w-full items-center gap-2 px-3 py-2 text-sm bg-[var(--primary)]/15 text-[var(--primary)] hover:bg-[var(--primary)]/25 font-medium"
                            onClick={() => handleAssignSelected(bulkRecommendation)}
                          >
                            {(() => {
                              const Icon = getCategoryIcon(bulkRecommendation);
                              return (
                                <>
                                  <Icon className="h-3.5 w-3.5 shrink-0 opacity-80" />
                                  {getCategoryLabel(bulkRecommendation)}
                                </>
                              );
                            })()}
                          </button>
              ...
              <Button
                variant="outline"
                size="sm"
                className="min-h-10 border-[var(--destructive)]/50 text-[var(--destructive)] hover:bg-[var(--destructive)]/20"
      ...
      <div className="max-h-[420px] overflow-y-auto overflow-x-hidden">
        {filtered.length === 0 ? (
          <div className="p-6 sm:p-8 text-center text-sm text-[var(--muted-foreground)]">
            No transactions match.
          </div>
        ) : (
          <>
            <div className="sticky top-0 z-10 flex items-center gap-2 px-3 sm:px-4 py-2.5 text-xs font-medium text-[var(--muted-foreground)] bg-[var(--card)] border-b border-[var(--border)]">
              <button
                type="button"
                className="flex items-center gap-2 min-h-10 hover:text-[var(--foreground)] touch-manipulation"
                onClick={selectAll}
              >
                {allSelected ? (
            ...
            <ul className="divide-y divide-[var(--border)]">
              {filtered.map((tx) => {
                ...
                const recommended = getRecommendedCategoryFromTransaction(tx);
                const showSuggested =
                  tx.category === "uncategorized" && recommended != null;
                return (
                  <li
                    key={tx.id}
                    className={`flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3 px-3 sm:px-4 py-3 sm:py-2.5 transition-colors duration-150 hover:bg-[var(--secondary)]/60 ${
                  ...
                  >
                    <div className="flex items-center gap-2 sm:gap-3 min-w-0 flex-1">
                      <button
                        ...
                        className="shrink-0 min-h-10 min-w-10 flex items-center justify-center text-[var(--muted-foreground)] hover:text-[var(--foreground)] transition-colors duration-150 touch-manipulation"
                      >
                        {selected ? (
                          <CheckSquare className="h-5 w-5 sm:h-4 sm:w-4 text-[var(--primary)]" />
                        ) : (
                    ...
                    <div className="flex items-center gap-2 shrink-0 pl-7 sm:pl-0 flex-wrap">
                      {showSuggested && (
                        <Button
                          variant="outline"
                          size="sm"
                          className="min-h-10 text-xs border-[var(--primary)]/50 text-[var(--primary)] hover:bg-[var(--primary)]/10 touch-manipulation"
                          onClick={() => handleAssignOne(tx.id, recommended)}
                          title={`Assign to suggested: ${getCategoryLabel(recommended)}`}
                        >
                          {getCategoryLabel(recommended)}
                        </Button>
                      )}
                      <select
                        value={tx.category}
                        ...
                        className="h-10 sm:h-8 min-h-10 rounded-md border border-[var(--border)] bg-[var(--secondary)] px-2 text-xs text-[var(--foreground)] transition-colors duration-150 focus:outline-none focus:ring-1 focus:ring-[var(--primary)] min-w-[100px] max-w-[140px] sm:max-w-[120px] touch-manipulation"
                        style={{
                          color:
                      ...
                      <Button
                        variant="ghost"
                        size="icon"
                        className="h-10 w-10 sm:h-8 sm:w-8 text-[var(--muted-foreground)] hover:text-[var(--destructive)] touch-manipulation"
